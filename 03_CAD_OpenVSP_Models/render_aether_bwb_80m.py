...
"""
=============================================================================
3D CAD Visualization Script: AETHER 80m Liquid Hydrogen Blended Wing Body
=============================================================================
...
Parses the generated OpenVSP STL surface mesh and produces 4 engineering views:
1. Isometric 3D Shaded Solid (Overall Blended Airframe, 3 Turbofans & Winglets)
2. Planform (Top View) with 430-Seat Theater Cabin & 6 Cryogenic LH2 Tanks (~798 m³)
3. Side Elevation (Centerbody Lifting Airfoil Profile t/c = 19.5% & Top Engines)
...
def render_aether_bwb_80m_views(
    oml_stl="/Users/jakkasaisrinivasamanideep/Documents/MMS236/03_CAD_OpenVSP_Models/AETHER_BWB_80m_OML.stl",
    output_img="/Users/jakkasaisrinivasamanideep/Documents/MMS236/03_CAD_OpenVSP_Models/aether_bwb_80m_3d_views.png"
):
    print("Loading AETHER 3D STL mesh...")
    tris = load_stl_mesh(oml_stl)
    if len(tris) == 0:
        print(f"Error: STL mesh not found at {oml_stl}")
        return

    print(f"Loaded OML Mesh: {len(tris)} triangular facets.")

    # Tailored aesthetic: Cyan-blue high-tech aerodynamic solid shading
    face_colors = compute_face_shading(tris, base_rgb=(0.12, 0.68, 0.96), light_dir=(0.35, -0.55, 0.75), alpha=0.94)
    ...
    # 6 Cryogenic LH2 Tanks (~798 m³ total capacity)
    tanks = [
        # (x, y, dx, dy, name)
        (16.0, -11.5, 3.3, 12.5),
        (16.0,  11.5, 3.3, 12.5),
    ...
    # 3 Geared Turbofan Engines (Center X=36.5 Y=0, Port X=36 Y=-6.2, Stbd X=36 Y=+6.2)
    engine_positions = [(0.0, 36.5), (-6.2, 36.0), (6.2, 36.0)]
    for ey, ex in engine_positions:
        eng_rect = patches.FancyBboxPatch((ey - 1.7, ex), 3.4, 7.0, boxstyle="round,pad=0.2,rounding_size=0.8",
    ...
    ax2.text(0, 36.0, "Aft Core LH₂ Tanks\n(797.6 m³ Total)", color='#a7f3d0', fontsize=7.5, fontweight='bold', ha='center', va='center', zorder=10)
    ax2.text(0, 50.0, "Triple Geared Turbofans (D = 3.4m, L = 7.0m)\nUpper Deck Acoustic & BLI Shielding", color='#fde047', fontsize=7.5, fontweight='bold', ha='center', va='center',
    ...
    ax3.text(39.5, 6.2, "Top Nacelles (Aft Deck)", color='#fde047', fontsize=7.5, fontweight='bold', ha='center', zorder=10)
    ...
    for ey, ez in [(-6.2, 3.8), (0.0, 4.2), (6.2, 3.8)]:
        eng_front = patches.Circle((ey, ez), 1.7, linewidth=1.2, edgecolor='#facc15', facecolor='#78350f', alpha=0.95, zorder=7)
    ...
    plt.suptitle('AETHER 80m Liquid Hydrogen Blended Wing Body (OpenVSP 3.51.3 Parametric CAD Model)\n'
                 'Group 13 | Wingspan: 80.0 m (ICAO Code F) | Length: 60.0 m | 430 Pax Unified Theater Cabin | 797.6 m³ Cryotanks',
    ...
    plt.close()
    print(f"[OK] Saved AETHER 3D CAD Multi-View: {os.path.abspath(output_img)}")

if __name__ == "__main__":
    render_aether_bwb_80m_views()
